...
# Output diarization_ensemble_csv
diarization_ensemble_output_path= os.path.join(diarization_file_path, 'ensemble', '{}.csv'.format(filename_noftype))
diarization_ensemble_csv.to_csv(diarization_ensemble_output_path, index = False)
...
for current_row_idx, row in diarization_ensemble_csv.iterrows():
    ...
    if row['speaker_change_ensemble']:
